...
public class CompositionLinkButtonAction extends AbstractAction{
	...
	public CompositionLinkButtonAction(){
	
		putValue(SMALL_ICON, new ResourceLoader().loadImageIcon("composition.png"));
		putValue(SHORT_DESCRIPTION, "Composition link");
		putValue(NAME, "Composition link");
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		((LinkState)state).setAssociationType(AssociationType.COMPOSITION); 
		MainFrame.getInstance().setStatusTrack(state.toString());
		context.setCurrentState(state);

		view.setSelectedTool(ToolSelected.COMPOSITION);
