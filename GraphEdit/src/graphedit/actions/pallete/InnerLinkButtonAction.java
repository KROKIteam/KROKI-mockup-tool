...
public class InnerLinkButtonAction extends AbstractAction{
	...
	public InnerLinkButtonAction(){
	
		putValue(SMALL_ICON, new ResourceLoader().loadImageIcon("inner_link.png"));
		putValue(SHORT_DESCRIPTION, "Inner Link ");
		putValue(NAME, "Inner Link ");
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		((LinkState)state).setLinkType(Link.LinkType.INNERLINK); 
		MainFrame.getInstance().setStatusTrack(state.toString());
		context.setCurrentState(state);

		view.setSelectedTool(ToolSelected.INNERLINK);
