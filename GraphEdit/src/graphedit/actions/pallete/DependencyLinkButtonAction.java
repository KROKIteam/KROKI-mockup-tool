...
public class DependencyLinkButtonAction extends AbstractAction{
	...
	public DependencyLinkButtonAction(){
	
		putValue(SMALL_ICON, new ResourceLoader().loadImageIcon("dependency.png"));
		putValue(SHORT_DESCRIPTION, "Dependency link");
		putValue(SHORT_DESCRIPTION, "Dependency link");
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		((LinkState)state).setLinkType(Link.LinkType.DEPENDENCY); 
		MainFrame.getInstance().setStatusTrack(state.toString());
		context.setCurrentState(state);

		view.setSelectedTool(ToolSelected.DEPENDENCY);
